...
CREATE UNIQUE INDEX IF NOT EXISTS enterprise_credit_code_uq
...
CREATE TABLE IF NOT EXISTS cooperation_demand (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  enterprise_id UUID NOT NULL REFERENCES enterprise(id) ON DELETE CASCADE,
...
CREATE TABLE IF NOT EXISTS ecosystem_match (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
...
CREATE INDEX IF NOT EXISTS enterprise_roles_gin ON enterprise USING gin (enterprise_roles);
CREATE INDEX IF NOT EXISTS enterprise_scenarios_gin ON enterprise USING gin (service_scenarios);
CREATE INDEX IF NOT EXISTS product_service_scenarios_gin ON product_service USING gin (scenarios);
CREATE INDEX IF NOT EXISTS cooperation_demand_scenarios_gin ON cooperation_demand USING gin (scenarios);
CREATE INDEX IF NOT EXISTS policy_published_on_idx ON policy_document (published_on DESC);
CREATE INDEX IF NOT EXISTS ecosystem_match_demand_score_idx ON ecosystem_match (demand_id, score DESC);
