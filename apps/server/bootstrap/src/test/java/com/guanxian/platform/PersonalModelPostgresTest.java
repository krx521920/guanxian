...
class PersonalModelPostgresTest {
    ...
    @Test void migratesAndPersistsEncryptedUpsertsWithOwnerIsolationAndDeletion() {
        ...
        // Exercise upgrades from the existing enterprise-account schema, not just an empty database.
        Flyway.configure().dataSource(datasource).target("28").load().migrate();
        var before = new JdbcTemplate(datasource);
        assertThat(before.queryForObject("SELECT to_regclass('public.personal_model_setting')::text", String.class)).isNull();
        Flyway.configure().dataSource(datasource).load().migrate();
        var jdbc = new JdbcTemplate(datasource);
        assertThat(jdbc.queryForObject("SELECT count(*) FROM flyway_schema_history WHERE version='29' AND success", Integer.class)).isEqualTo(1);
