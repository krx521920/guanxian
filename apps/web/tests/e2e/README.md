...
- 正式 Excel 调查模板下载、填写、预检、提交和协会审核；
- 产品跨协会逐资源授权与立即撤销；
- 附件入知识库、审核发布、带出处问答和原始附件下载校验。
...
CI 使用单 worker、零重试运行全部 Playwright 业务旅程并上传 HTML、截图、录像和 trace。任何旅程失败都不得以重试后的结果覆盖第一次失败；本地是否通过以本次实际报告为准，不沿用历史口头结论。
