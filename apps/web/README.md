...
- 会员企业：检索、状态筛选、资料完整度、编辑与 ETag 并发冲突保护
- 政策标准：分级筛选、影响摘要与政策列表
...
| PUT | `/api/v1/members/{id}` | 携带 `If-Match` 更新；陈旧版本返回 412 |
...
| `src/services/platform-api.test.ts` | 强 ETag 读取、If-Match 写入、缺失 ETag 拒绝和 412 冲突保留 |
...
当前基线（2026-08-20）：109 项 Vitest 测试全部通过；Stryker 对 510 个核心逻辑变异进行验证，杀死 494 个、存活 13 个、无覆盖 3 个，总变异得分 96.86%，HTTP 客户端为 100%，认证模块为 91.49%。报告以本次 `npm run mutation` 输出及本地 `reports/mutation/` 产物为准。
