...
const dirty = computed(() => !settings.value?.saved || apiKey.value !== ''
  ...
  || enabled.value !== settings.value.saved.enabled)
const canSave = computed(() => settings.value?.storageAvailable && !busy.value && !!provider.value
  ...
  && (!!apiKey.value.trim() || retainedKey.value) && (!enabled.value || consent.value))
...
function apply(value: ModelSettings) {
  ...
  enabled.value = value.saved?.enabled ?? true
  apiKey.value = ''
}
...
async function save() {
  ...
  await run(async signal => {
    const value = await personalModelApi.save({ provider: provider.value, model: model.value.trim(),
      apiKey: apiKey.value, enabled: enabled.value, externalDataConsent: consent.value }, signal)
    if (signal.aborted) return
    apply(value)
    notice.value = '已保存。下次提问使用新设置，并开始新的会话上下文。'
    emit('changed', value)
  })
  apiKey.value = ''
}
...
async function testConnection() {
  if (busy.value || dirty.value || !consent.value || !settings.value?.egressAllowed) return
  testResult.value = null
  await run(async signal => {
    const value = await personalModelApi.test(consent.value, signal)
    ...
    <div class="model-settings-backdrop" @click.self="close">
      <section ref="dialog" class="model-settings-panel" role="dialog" aria-modal="true" aria-labelledby="model-settings-title" tabindex="-1" @keydown="handleKey">
        ...
        <div class="model-settings-body">
          <p class="model-intro">将您自己的大模型连接到管线智能助手。配置保存在当前账号下，不会改变同事的设置。</p>
          <p v-if="busy && !settings" role="status">正在读取个人配置…</p>
          <template v-if="settings">
            <p v-if="!settings.storageAvailable" class="model-warning" role="status">管理员尚未配置密钥加密服务，暂不能保存或连接。现有聊天仍可使用。</p>
            <p v-if="!settings.egressAllowed" class="model-warning" role="status">平台尚未允许模型数据外发。您可以保存配置，但连接测试与个人模型聊天需管理员开启后才生效。</p>
            <form id="personal-model-form" class="model-form" @submit.prevent="save">
              ...
              <small>密钥仅通过服务器调用官方接口，加密保存、不回显，也不保存在浏览器中。网页会员订阅不等于 API 余额。</small>
              <label class="model-check"><input v-model="enabled" type="checkbox" :disabled="busy" /><span>在我的聊天中优先使用此模型<small>关闭后使用平台默认模式，保留已保存的配置。</small></span></label>
              <label class="model-check model-consent"><input v-model="consent" type="checkbox" :disabled="busy" /><span>我了解并同意数据外发与 API 计费<small>提问、当前会话上下文、可见知识片段与只读业务查询结果可能发送给所选厂商。请确认组织允许；费用由该厂商计入我的 API 账号。连接测试只发送测试文本，不携带业务资料。</small></span></label>
            </form>
        ...
        <footer class="model-settings-actions">
          ...
          <button type="button" class="model-secondary" :disabled="busy || dirty || !consent || !settings?.egressAllowed || !settings?.storageAvailable" @click="testConnection">测试连接</button>
          <button type="submit" form="personal-model-form" class="model-primary" :disabled="!canSave">{{ busy ? '处理中…' : '保存设置' }}</button>
        </footer>
      </section>
    </div>
  </Teleport>
...
.model-settings-panel { width: min(560px, 100%); max-height: calc(100dvh - 40px); display: flex; flex-direction: column; border: 1px solid var(--line, #d8e3e7); border-radius: 20px; background: var(--panel, #fff); color: var(--ink, #18333c); box-shadow: 0 24px 80px rgb(8 28 40 / .26); outline: none; overflow: hidden; }
.model-settings-header { padding: 24px 26px 18px; display: flex; justify-content: space-between; align-items: flex-start; border-bottom: 1px solid var(--line); }
.model-settings-eyebrow { font-size: 11px; color: var(--primary); letter-spacing: .05em; }
.model-settings-header h2 { margin: 6px 0 0; font-size: 24px; }
.model-close { border: 0; background: transparent; font-size: 28px; color: var(--muted); width: 36px; height: 36px; cursor: pointer; }
.model-settings-body { min-height: 0; overflow-y: auto; overscroll-behavior: contain; padding: 0 26px 18px; }
.model-intro { font-size: 13px; line-height: 1.8; color: var(--muted); margin: 18px 0; }
.model-form { display: grid; gap: 9px; }
.model-form > label { font-size: 13px; font-weight: 700; margin-top: 6px; }
.model-form > input, .model-form > select { min-height: 44px; width: 100%; box-sizing: border-box; border: 1px solid var(--line); border-radius: 9px; background: var(--panel); color: var(--ink); padding: 10px 12px; font: inherit; font-size: 13px; }
...
.model-check { display: flex; align-items: flex-start; gap: 9px; cursor: pointer; }
.model-check input { margin-top: 3px; accent-color: var(--primary); flex-shrink: 0; }
.model-check small { display: block; margin-top: 4px; }
.model-consent { padding: 12px; border: 1px solid var(--line); border-radius: 10px; }
...
.model-settings-actions { border-top: 1px solid var(--line); padding: 16px 26px; display: flex; justify-content: flex-end; gap: 10px; flex-wrap: wrap; }
