...
<template>
  <div>
    ...
    <section v-else class="collaboration-list"><article v-for="item in filtered" :key="item.id" class="collaboration-card panel" :class="{ 'is-overdue': overdueDays(item) > 0 }"><div class="collab-id">{{ shortCollaborationId(item.id) }}</div><div class="collab-main"><div class="collab-title"><StatusBadge :value="displayBusinessStatus(item.priority || '')" /><h2>{{ item.title }}</h2></div><p class="participants"><span v-for="participant in item.participants" :key="participant">{{ participant }}</span><span v-if="!item.participants.length">待确认参与方</span></p><div class="collab-progress"><div><span>当前阶段 · {{ displayBusinessStatus(item.stage) }}</span><strong>{{ item.progress }}%</strong></div><div class="progress-track"><i :style="{ width: `${item.progress}%` }" /></div></div></div><div class="collab-next"><span>下一步行动</span><strong>{{ item.nextAction || '待确定' }}</strong><small :class="{ overdue: overdueDays(item) > 0 }">负责人：{{ item.owner || '未指定' }} · 截止 {{ item.dueDate || '未设置' }}<template v-if="overdueDays(item) > 0"> · 已逾期 {{ overdueDays(item) }} 天</template></small></div><button class="secondary-button small" @click="openDetail(item)">进入协作</button></article><div v-if="!filtered.length" class="panel empty-business-state"><b>暂无协作事项</b><span>确认的匹配可直接发起协作，也可独立建立协会事项。</span></div><PaginationBar :page="page" :size="size" :total="total" :disabled="loading" @change="changePage" @resize="resizePage" /></section>
