...
const canBulkReview = computed(() => ['SYSTEM_ADMIN', 'ASSOCIATION_ADMIN'].includes(auth.user.value?.role || ''))
...
<template>
  <div>
    ...
    <section v-else class="panel flush-panel">
      ...
      <div v-else class="empty-business-state"><b>暂无符合条件的会员企业</b><span>可以调整关键词或状态筛选后重试，也可以新增或批量导入企业资料。</span><div class="inline-actions"><button class="secondary-button small" type="button" @click="clearFilters">清除筛选</button><RouterLink v-if="canCollect" class="primary-button small" to="/members/new">+ 新增企业</RouterLink></div></div>
