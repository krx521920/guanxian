...
export const platformApi = {
  ...
  reviewAssociationAccessRequest: (item: AssociationAccessRequest, approved: boolean, comment: string) => request<AssociationAccessRequest>(`/cross-associations/access-requests/${encodeURIComponent(item.id)}/review`, { method: 'PUT', body: JSON.stringify({ decision: approved ? 'APPROVE' : 'REJECT', comment: comment.trim() || null, allowMemberData: approved }) }),
  associationRelationships: () => request<AssociationRelationship[]>('/cross-associations/relationships'),
  changeAssociationRelationship: (item: AssociationRelationship, action: 'ACTIVATE' | 'SUSPEND' | 'REVOKE', reason: string) => request<AssociationRelationship>(`/cross-associations/relationships/${encodeURIComponent(item.sourceAssociationId)}/${encodeURIComponent(item.targetAssociationId)}`, { method: 'PUT', headers: { 'If-Match': etag(item.version) }, body: JSON.stringify({ action, reason: reason.trim() || null }) }),
