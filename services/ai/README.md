...
北京地下管线协会 AI 管理协作平台的独立 AI 服务。当前版本不调用外部模型，使用确定性规则完成企业资料结构化、供需匹配和政策文本检索，便于前后端联调与自动化测试。
...
| POST | `/api/v1/qa/policy` | 从随请求传入的政策材料中检索相关段落（占位 RAG） |
...
`pytest` 默认运行单元、接口、反向、边界、异常和 Hypothesis 属性/模糊测试，
自动排除耗时的高压测试。Hypothesis 固定为可重现模式，常规测试适合本地和 CI 频繁执行。

当前基线（2026-08-15）：47 项常规测试全部通过；应用整体覆盖率 96%，请求上下文模块为 100%，企业结构化、供需匹配和政策问答三个核心服务覆盖率为 97%–100%。外部 Schemathesis 基于 OpenAPI 生成的 681 个请求全部通过。

### 独立高压测试

高压用例使用 16 个线程并发执行 2,000 轮，每轮同时覆盖企业结构化、供需匹配和政策问答：
...
后续接入模型时，应保留现有 Pydantic 输入输出契约，在服务层替换实现，并补充知识库版本、召回来源、模型版本和全链路审计字段。
