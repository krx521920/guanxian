...
### V13–V17 身份、租户与跨协会完整性迁移闸门

V13–V17 属于有意收紧写入契约的迁移，不能假定执行后的数据库可由旧镜像安全写入：
...
上线变更单必须先统计未知用户状态、无归属政策、无归属订阅、跨协会政策影响，以及 V17 列出的所有无效授权/推荐/关系/策略记录，附上逐项处置清单，并在生产数据隔离副本完成 V12→V17 迁移回归。V16/V17 的异常包含样本 `DETAIL` 和处置 `HINT`；出现失败必须保留原版本、审核并修正业务数据后重试，禁止使用 `flyway repair`、手工修改迁移历史或未经审核的批量清洗绕过。

V17 不会在迁移时把单个“截止时间已过但状态仍为 `ACTIVE`”的旧授权静默改成 `EXPIRED`；同一资源发生首次重新授权时，触发器才会在同一事务内物化旧记录，并让唯一索引串行化并发请求。若升级前同一授权键已有多条 `ACTIVE` 记录，即使均已过期也会 fail-fast，必须先由业务负责人确认保留、撤销或过期哪一条。V13–V17 一旦在生产执行，回滚目标必须是已经声明兼容这些数据库契约的镜像；若应用异常，优先发布向前修复。只有经过 DBA 演练和业务负责人批准的数据恢复，才能回到迁移前备份。
...
2. 先执行第 2 节迁移兼容判断。V13–V17 不得视为可直接回滚的普通向前兼容变化；V13–V15 禁止直接回滚至 V12，V16/V17 也只能切换到兼容矩阵明确验证过同协会影响和跨协会约束的镜像，才可保持数据库不变并使用上一审批镜像 digest。
...
特别说明：执行过 V13–V17 的数据库禁止直接启动 V12 或更早应用镜像，也禁止启动仍可能写入跨协会政策影响、空资源授权或非法跨协会生命周期的未验证镜像。旧通知仓储不了解协会级唯一键，旧政策写入不了解强制协会上下文，旧跨协会写入也不了解 V16/V17 数据库不变量；直接回滚会产生持续写入失败或重新引入越权风险。此时只允许切换到已经声明兼容 V13–V17 的镜像，或采用第 3 步的向前修复。
