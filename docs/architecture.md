...
Web Chat Agent 使用 Spring AI 1.1.x 统一模型接入、会话记忆和工具调用，但不会绕过现有 Java 权限与 RAG 链路。`/api/v1/assistant/chat` 提供兼容的完整响应，`/api/v1/assistant/chat/stream` 以 `start`、`delta`、`complete`、`error` 结构化事件实时返回并支持客户端取消；生产 Nginx 对该精确路径关闭响应缓冲。两条链路都必须先解析 `ActorScope` 并在所选协会内检索已发布、当前身份可见的知识片段，再把带编号的引用证据交给模型；没有外部模型时仍返回本地带出处回答。会话记忆以“用户主体 + 协会 + 前端会话 UUID”的摘要作为隔离键，单会话最多保留 12 条消息，进程内最多保留 500 个最近会话，重启即清空。

Agent 当前只开放六个只读工具：页面帮助，以及会员企业、产品服务、合作需求、生态匹配、协作事项查询。业务工具调用复用 HTTP 接口相同的权限名和领域服务，由服务端通过非模型参数的 `ToolContext` 注入身份与协会范围；模型不能声明或覆盖权限。会员工具会剔除联系人、电话、邮箱、统一信用代码等敏感字段，匹配列表使用严格只读路径，不在查询时自动修改过期状态。页面元数据、知识片段和工具结果均按不可信输入处理，所有工具都不能触发新增、修改、删除、审批、导入或外部系统操作。
