...
`member` 模块通过内部仓储端口访问会员快照，默认适配器为 PostgreSQL；进程内仓储只在显式测试配置下创建。信用代码唯一性由部分唯一索引兜底，更新与删除使用 `WHERE id = ? AND version = ?` 的数据库 CAS。Flyway 以版本 0 为既有非空数据库建立基线，再按 V1/V2 迁移；已发布迁移不可重写，后续变更只能追加新版本。

生产认证是无状态 OIDC/JWT 资源服务器。后端校验 issuer、JWK 签名和白名单角色/权限，并禁止生产 Profile 启用 demo。Web 采用 Authorization Code + PKCE，从后端 `/users/me` 回读最终身份；页面角色只影响导航可见性，接口授权始终由后端负责。会员编辑页面必须先读取强 ETag，再以 `If-Match` 更新；412 表示数据已被他人更新，客户端必须重新加载。
