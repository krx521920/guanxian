...
Web Chat Agent 使用 Spring AI 1.1.x 统一模型接入、会话记忆和工具调用，但不会绕过现有 Java 权限与 RAG 链路。`/api/v1/assistant/chat` 必须先解析 `ActorScope` 并在所选协会内检索已发布、当前身份可见的知识片段，再把带编号的引用证据交给模型；没有外部模型时仍返回本地带出处回答。会话记忆以“用户主体 + 协会 + 前端会话 UUID”的摘要作为隔离键，单会话最多保留 12 条消息，进程内最多保留 500 个最近会话，重启即清空。当前只开放 `current_page_help` 只读工具，页面、知识片段和工具结果都按不可信输入处理，不能触发新增、修改、删除、审批、导入或外部系统操作。
