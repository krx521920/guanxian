...
`member` 模块通过内部仓储端口访问会员快照，默认适配器为 PostgreSQL；进程内仓储只在显式测试配置下创建。信用代码唯一性由部分唯一索引兜底，更新与删除使用 `WHERE id = ? AND version = ?` 的数据库 CAS。Flyway 以版本 0 为既有非空数据库建立基线，当前迁移链为 V1–V15；已发布迁移不可重写，后续变更只能追加新版本。
